...
static void xml_document_dtor(vv_object_t*);
...
static void xml_node_list_dtor(vv_object_t* self);
...
static void xml_node_list_iter_dtor(vv_object_t* self);
...
static void
xml_document_dtor(vv_object_t* self)
{
  xmlDoc* doc;
...
xml_node_all(vv_object_t* self, vv_object_t* name)
{
  ...
  return vv_new_object(xml_type_node_list, args, 2);
}
...
static void
xml_node_list_dtor(vv_object_t* self)
{
  xml_c_node_list* node_list;
  int success = vv_get_blob(self, (void**)&node_list);
  if (success == 0 && node_list) {
...
static void
xml_node_list_iter_dtor(vv_object_t* self)
{
