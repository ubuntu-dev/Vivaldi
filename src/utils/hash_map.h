...
/// (e.g.\ most value::base::member hashes will contain at most one or two
/// items), this class's extremely simple rehashing heuristic ends up being a
/// net win.
...
public:
  ...
  /// Returns an iterator pointing to the element with key `item`.
  /// \returns An iterator pointing to an element with key `item`, or end() if
  ...
  /// If an element with key `item` already exists, it's overwritten. This
  /// function should run in O(1), but can run in O(n) if using a pathologically
  /// bad hashing algorithm or if a rehash is needed.
  /// If an element with key `item` is overwritten, all iterators are preserved;
  /// otherwise, all iterators are invalidated by this method.
  ...
  /// Returns the value at key `item`.

  /// If no item exists at key `item`, an std::out_of_range error is thrown.
