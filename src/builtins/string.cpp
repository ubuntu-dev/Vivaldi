...
{
  vm.self();
  auto& str = static_cast<value::string&>(*vm.retval);
  vm.arg(0);
  auto arg = vm.retval;
...
{
  vm.self();
  auto sz = static_cast<value::string&>(*vm.retval).val.size();
...
value::base* fn_string_equals(vm::machine& vm)
{
  ...
  vm.self();
  return gc::alloc<value::boolean>( to_string(vm.retval) == to_string(arg) );
}
...
value::base* fn_string_add(vm::machine& vm)
{
  ...
  auto str = to_string(other);
  vm.self();
  auto new_str = static_cast<value::string&>(*vm.retval).val + str;
...
value::base* fn_string_times(vm::machine& vm)
{
  ...
  vm.self();
  auto val = static_cast<value::string&>(*vm.retval).val;
  std::string new_str{};
  for (auto i = to_int(other); i--;)
    new_str += val;
  return gc::alloc<value::string>( new_str );
}
...
{
  vm.self();
  return gc::alloc<value::integer>(vv::to_int(to_string(vm.retval)));
}
...
value::base* fn_string_at(vm::machine& vm)
{
  ...
  vm.self();
  const auto& str = static_cast<value::string&>(*vm.retval).val;
...
{
  vm.self();
  auto& self = static_cast<value::string&>(*vm.retval);
...
{
  vm.self();
  auto iter = static_cast<value::string_iterator*>(vm.retval);
...
value::base* fn_string_iterator_equals(vm::machine& vm)
{
  ...
  auto iter = static_cast<value::string_iterator*>(vm.retval);
  vm.arg(0);
  auto other = static_cast<value::string_iterator*>(vm.retval);
